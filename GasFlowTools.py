...
def extract_fof(path,mcut,snapidxmin=0):
    ...
    for isnap,groupdir in enumerate(groupdirs):
        ...
        if snapidx>=snapidxmin:
            logging.info(f'Processing snap {snapidx+1} ({isnap}/{len(groupdir)}) [runtime {time.time()-t0:.2f} sec]')
            ...
            for ifile_snap, groupdirfname in enumerate(groupdirfnames):
                ...
                logging.info(f'Snap {snapidx} ({isnap}/{len(groupdir)}), file {ifile_snap+1}/{groupdirfnames_n}: extracting data for {ifile_nfof:.0f} FOFs [runtime {time.time()-t0:.2f} sec]')
